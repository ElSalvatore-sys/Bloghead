    ...
    <div className={`inline-flex items-center bg-bg-card border border-white/10 rounded-lg p-1 ${className}`}>
      ...
      >
        {view === 'grid' && (
          <motion.div
            layoutId="viewToggle"
            className="absolute inset-0 bg-accent-purple rounded-md"
            transition={{ type: 'spring', bounce: 0.2, duration: 0.4 }}
          />
        )}
        <Grid3X3 className="w-4 h-4 relative z-10" />
        <span className="relative z-10 hidden sm:inline">Raster</span>
      </button>
      ...
      >
        ...
        <span className="relative z-10 hidden sm:inline">Karte</span>
      </button>
    </div>
  )
}
